-- The purpose of query  - to review existing grantees privileges 
...
SELECT SA.GRANTOR, SA.GRANTEE, SA.TTNAME ,SA.ALTERAUTH, SA.DELETEAUTH, SA.INDEXAUTH ,SA.INSERTAUTH, 
  SA.SELECTAUTH, SA.UPDATEAUTH , "not_OK" AS OK_STATUS, strftime( datetime(current_timestamp, 'localtime')) AS QUERY_TS 
...
UNION
SELECT SA.GRANTOR, SA.GRANTEE, SA.TTNAME, SA.ALTERAUTH, SA.DELETEAUTH, SA.INDEXAUTH, SA.INSERTAUTH, 
  SA.SELECTAUTH, SA.UPDATEAUTH, "OK" AS OK_STATUS,  strftime( datetime(current_timestamp, 'localtime'))  AS QUERY_TS 
  FROM SYSTABAUTH SA, TABLES_TO_CHECK TTC, GRANTEES_PRIVILEGES_ALLOWED GPTC, GRANTEES_PRIVILEGES_EXIST GF
    WHERE 
      SA.TTNAME = TTC.TABNAME

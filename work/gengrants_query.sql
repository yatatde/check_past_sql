...
with TABLES_GRANTS_REQUIRED (GRANTEES, TABLEPREFIX, SELECTAUTH, UPDATEAUTH, DELETEAUTH, INSERTAUTH) as
...
-- select 'CRANTEE_MUST_E', 'TABPR',          '',        '',       '',       '' from SYSDUMMY1 
  ...
  TABLES_GRANTS_MUST_HAVE (GRANTEES, TABLEPREFIX, SELECTAUTH, UPDATEAUTH, DELETEAUTH, INSERTAUTH) as
  (
...
as (select STABAUTH.TCREATOR, STABAUTH.TTNAME, STABAUTH.GRANTEES,
...
from TABLES_GRANTS_EXIST as TG_EXIST inner join  TABLES_GRANTS_MUST_HAVE as TG_MUST_HAVE 
  on    
    ...
    and TG_MUST_HAVE.TABLEPREFIX = (select max(MH.TABLEPREFIX) 
                                    ...
                                    group by MH.GRANTEES)   
  group by TG_EXIST.TTNAME, TG_MUST_HAVE.GRANTEES, TG_MUST_HAVE.TABLEPREFIX,
...
union select "--- generated at " || CURRENT_TIME ||', ' || CURRENT_DATE from SYSDUMMY1 as command